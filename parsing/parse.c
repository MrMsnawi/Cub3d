/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parse.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: abmasnao <abmasnao@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/09 10:11:25 by abmasnao          #+#    #+#             */
/*   Updated: 2025/12/12 16:19:38 by abmasnao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/cub.h"

void	path_parse(char *path)
{
	int		pfd;
	size_t	len;

	if (!path)
		exit_error("Error: Something went wrong!\n");
	len = ft_strlen(path);
	pfd = open(path, O_RDONLY);
	if (pfd < 0)
		exit_error("Error: (check existance|read permission)\n");
	close(pfd);
	if (len < 5 || !valid_char_name(path[len - 5]))
		exit_error("Error: invalid name!\n");
	if (path[len - 4] != '.' || path[len - 3] != 'c'
		|| path[len - 2] != 'u' || path[len - 1] != 'b')
		exit_error("Error: invalid extension! (.cub)\n");
}

char	*file_content(char *buf)
{
	char	*ret;
	int		j;

	ret = ft_malloc(ft_strlen(buf) + 1);
	if (!ret)
		exit_error("Error: ft_malloc!\n");
	j = -1;
	while (buf[++j])
		ret[j] = buf[j];
	ret[j] = '\0';
	return (ret);
}

char	*get_file_content(int fd)
{
	char	c;
	int		i;
	char	*buf;
	int		counter;

	i = 0;
	buf = NULL;
	counter = 1;
	while (counter > 0)
	{
		buf = ft_realloc(buf, i + 2);
		if (!buf)
			return (close(fd), exit_error("Error: ft_realloc failed!\n"), NULL);
		counter = read(fd, &c, 1);
		if (counter == 0)
			break ;
		if (counter < 0)
			return (close(fd), \
exit_error("Error: read failed!\n"), NULL);
		buf[i] = c;
		buf[i + 1] = '\0';
		i++;
	}
	close(fd);
	return (file_content(buf));
}

void	file_read(t_data *data, char *path)
{
	int		m_fd;
	char	*r_file;

	if (!data || !path)
		exit_error("Error: Something went wrong!\n");
	m_fd = open(path, O_RDONLY);
	if (m_fd < 0)
		exit_error("Error: can't open the map!\n");
	r_file = get_file_content(m_fd);
	if (!r_file)
		exit_error("Error: can't read the map!\n");
	if (is_splitted_map(r_file))
		exit_error("Error: invalid map!\n");
	data->utils.file_data = ft_split(r_file, '\n');
	if (!data->utils.file_data)
		exit_error("Error: splitting issue\n");
}

void	parse(t_data *data, char *path)
{
	if (!path)
		exit_error("Error: Something went wrong!\n");
	path_parse(path);
	file_read(data, path);
	element_parse(data);
}
